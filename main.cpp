...
short state=0;//0:playing   1:win   -1:lose
class bullet_class
{
public:
    int allive = 0 ;
...
bool collision(sf::Sprite ship,sf::Sprite bullet,bool is_enemy)
{
    if (is_enemy)
    {
...
bool bullet_explode_enemy(sf::Sprite sp[enemy_count],int *bullet_is_alive,sf::Sprite bullet_in,bool EnemyIsAlive[enemy_count])
{
    ...
    if(*bullet_is_alive==1)
    {
        ...
        for (int i=0; i<enemy_count; i++)
        {
            ...
            if(EnemyIsAlive[i])
                if(collision(sp[i],bullet_in,true))
                {
...
bool bullet_explode_ship(sf::Sprite My_ship,sf::Sprite bullet_in,int *bullet_is_alive) //(sf::Sprite sp[enemy_count],int *bullet_is_alive,sf::Sprite bullet_in,bool EnemyIsAlive[enemy_count])
{
    if(*bullet_is_alive==1)
    {
        float bulletX=bullet_in.getPosition().x;
        ...
        float EnemyX=My_ship.getPosition().x;
        float EnemyY=My_ship.getPosition().y;
        // std::cout<<"bx: "<<bulletX<<" bY: "<<bulletY<<" eX: "<<EnemyX<<" EY: "<<EnemyY<<" a"<<i<<": "<<EnemyIsAlive[i]<<std::endl;
        if(ShipLives>0)
            if(collision(My_ship,bullet_in,false))
            {
                ShipLives--;
                *bullet_is_alive=0;
                return true ;
...
void  title_init ( char s[], bool enemy_is_alive1[],int bullet_count,short lives)
{
    int enemy_score=0;
    for(int i =0; i<enemy_count; i++)
    {
    ...
    sprintf(s,"Score : %d      shot : %d       lives : %d",enemy_score,bullet_count,lives);
    if (enemy_score==30)
        state=1;
    if (lives<=0)
        state=-1;
...
int main()
{
    int counter_lose = 0, counter_win = 0;
    auto t=time(NULL);
    srand(t);
    // Create the main window

    bool is_writen=false;
    std::ofstream myfile_time("time.txt");
    sf::RenderWindow window(sf::VideoMode(800,600),"Betrayal Game");
    sf::Font vazir_font;
    sf::Text score;
    sf::Texture bullet_texture, bullet_texture_enemy, background, background_win, background_lose;
    if(!bullet_texture.loadFromFile("images/bullet_main_3.png") || !bullet_texture_enemy.loadFromFile("images/bullet_main_3_enemy.png")||!background.loadFromFile("images/background.jpg")||!background_lose.loadFromFile("images/gameoverbckg.png")||!background_win.loadFromFile("images/youwinbckg.png"))
        return -1;
    ...
    sf::Sprite background_sprite_win (background_win);
    sf::Sprite background_sprite_lose (background_lose);
    sf::SoundBuffer shot, explode, explodemyship , wining;
    if (!shot.loadFromFile("sounds/shot.wav")&&!explode.loadFromFile("sounds/explosion.wav")&&!explodemyship.loadFromFile("sounds/explode_Myship.wav")&&!wining.loadFromFile("sounds/wining.wav"))
        return -1;
    sf::Sound shot_sound, explode_sound, explodemyship_sound , wining_sound ;
    shot_sound.setBuffer(shot);
    explode_sound.setBuffer(explode);
    explodemyship_sound.setBuffer(explodemyship);
    wining_sound.setBuffer(wining);
    sf::SoundBuffer background_music;
    if (!background_music.loadFromFile("sounds/background.wav"))
        return -1; // error
    sf::Sound background_music_sound ;
    background_music_sound.setBuffer(background_music);
    ...
    while (window.isOpen())
    {
        ...
        if (state==-1)
        {
            ...
            if(counter_lose == 0)
            {
                explodemyship_sound.play();
                counter_lose++;
            }

            window.clear();
            window.draw(background_sprite_lose);
            window.display();
        }
        else if (state==1)
        {
            ...
            window.clear();
            window.draw(background_sprite_win);
            window.display();
        }
        else
        {
            // bullet enemy receive pos
            short rnd_num=rand()%30;
            if (bulletEnemy1.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy1.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy2.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy2.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy3.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy3.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy4.allive ==0 &&Enemy_is_alive[rnd_num])
            {
                bulletEnemy4.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy5.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy5.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy6.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy6.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy7.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy7.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy8.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy8.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy9.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy9.get_pos(sprite[rnd_num].getPosition());
            }
            rnd_num=rand()%30;
            if (bulletEnemy10.allive == 0&&Enemy_is_alive[rnd_num])
            {
                bulletEnemy10.get_pos(sprite[rnd_num].getPosition());
            }
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
            {
                ...
                if (spriteMyShip.getPosition().x>0)
                spriteMyShip.move(-.3f, 0.f);
            }
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right))
            {
                // left key is pressed: move our character
                if (spriteMyShip.getPosition().x<800)
                spriteMyShip.move(+.3f, 0.f);
            }
            ...
            char title[500];
            title_init(title,Enemy_is_alive,bullet_count,ShipLives);
            score.setString(title);

            if(bullet_explode_enemy(sprite,&bullet1.allive,bullet1.bullet_sprite,Enemy_is_alive))
                explode_sound.play();
            if(bullet_explode_enemy(sprite,&bullet2.allive,bullet2.bullet_sprite,Enemy_is_alive))
                explode_sound.play();
            if(bullet_explode_enemy(sprite,&bullet3.allive,bullet3.bullet_sprite,Enemy_is_alive))
                explode_sound.play();

            if(bullet_explode_ship(spriteMyShip,bulletEnemy1.bullet_sprite,&bulletEnemy1.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy2.bullet_sprite,&bulletEnemy2.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy3.bullet_sprite,&bulletEnemy3.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy4.bullet_sprite,&bulletEnemy4.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy5.bullet_sprite,&bulletEnemy5.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy6.bullet_sprite,&bulletEnemy6.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy7.bullet_sprite,&bulletEnemy7.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy8.bullet_sprite,&bulletEnemy8.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy9.bullet_sprite,&bulletEnemy9.allive))
                explode_sound.play();
            if(bullet_explode_ship(spriteMyShip,bulletEnemy10.bullet_sprite,&bulletEnemy10.allive))
                explode_sound.play();
